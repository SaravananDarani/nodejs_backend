...
models.add = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                sqlapi.insertData('INSERT INTO mst_colume (name,title,subtitle,img,status,webid) VALUES (:name,:title,:subtitle,:img,1,:webid)', dataJson)
...
models.update = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                sqlapi.updateData('UPDATE mst_colume SET name =:name, status = 1,title =:title,subtitle =:subtitle,img=:img WHERE id = :dataId', dataJson)
                    .then(function (results) {
                        var reslog = {
                            status: 'success',
...
models.delete = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                sqlapi.deleteData('DELETE FROM mst_colume WHERE id=:id', dataJson)
                    .then(function (results) {
                        var reslog = {
                            status: 'success',
...
models.get = (req) => {
    ...
    sqlapi.selectData('SELECT * FROM  mst_colume ' + whereCondition, dataJson)
...
models.getId = (req) => {
    ...
    try {
        ...
        sqlapi.selectData('SELECT name AS sliderName,title AS sliderTitle,subtitle AS sliderSubtitle,img AS sliderImg FROM  mst_colume ' + whereCondition, dataJson)
            .then(function (results) {
                finalData = results;
