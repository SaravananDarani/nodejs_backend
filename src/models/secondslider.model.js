...
models.add = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                sqlapi.insertData('INSERT INTO mst_secondslider (name,title,subtitle ,status,webid) VALUES (:name,:title,:subtitle,1,:webid)', dataJson)
...
models.update = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                sqlapi.updateData('UPDATE mst_secondslider SET name =:name, status = 1,title =:title,subtitle =:subtitle  WHERE id = :dataId', dataJson)
                    .then(function (results) {
                        var reslog = {
                            status: 'success',
...
models.delete = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                sqlapi.deleteData('DELETE FROM mst_secondslider WHERE id=:id', dataJson)
                    .then(function (results) {
                        var reslog = {
                            status: 'success',
...
models.get = (req) => {
    ...
    sqlapi.selectData('SELECT * FROM  mst_secondslider ' + whereCondition, dataJson)
...
models.getId = (req) => {
    ...
    try {
        ...
        sqlapi.selectData('SELECT name AS sliderName,title AS sliderTitle,subtitle AS sliderSubtitle FROM  mst_secondslider ' + whereCondition, dataJson)
            .then(function (results) {
                finalData = results;
