...
models.add = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                sqlapi.insertData('INSERT INTO mst_client (name,img,status,webid) VALUES (:name,:img,1,:webid)', dataJson)
...
models.update = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                sqlapi.updateData('UPDATE mst_client SET name =:name, status = 1,img=:img WHERE id = :dataId', dataJson)
                    .then(function (results) {
                        var reslog = {
                            status: 'success',
...
models.delete = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                sqlapi.deleteData('DELETE FROM mst_client WHERE id=:id', dataJson)
                    .then(function (results) {
                        var reslog = {
                            status: 'success',
...
models.get = (req) => {
    ...
    sqlapi.selectData('SELECT * FROM  mst_client ' + whereCondition, dataJson)
...
models.getId = (req) => {
    ...
    try {
        ...
        sqlapi.selectData('SELECT name AS sliderName, img AS sliderImg FROM  mst_client ' + whereCondition, dataJson)
            .then(function (results) {
                finalData = results;
