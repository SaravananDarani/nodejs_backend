...
models.add = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                sqlapi.insertData('INSERT INTO mst_abouts (name,title,subtitle,img,status,webid) VALUES (:name,:title,:subtitle,:img,1,:webid)', dataJson)
...
models.update = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                if (dataJson.dataId) {
                    sqlapi.updateData('UPDATE mst_abouts SET   status = 1,title =:title,subtitle =:subtitle,img=:name WHERE id = :dataId', dataJson)
                        .then(function (results) {
                            var reslog = {
                                status: 'success',
                    ...
                    sqlapi.insertData('INSERT INTO mst_abouts(title,subtitle,img,status,webid) VALUES (:title,:subtitle,:name,1,:webid)', dataJson)
                        .then(function (results) {
                            if (results.length > 0) {
                                var reslog = {
                                    status: 'success',
...
models.delete = (req) => {
    ...
    Q.allSettled(promises)
        .then(result => {
            ...
            if (msg.length == 0) {
                sqlapi.deleteData('DELETE FROM mst_abouts WHERE id=:id', dataJson)
                    .then(function (results) {
                        var reslog = {
                            status: 'success',
...
models.get = (req) => {
    ...
    sqlapi.selectData('SELECT * FROM  mst_abouts ' + whereCondition, dataJson)
...
models.getId = (req) => {
    ...
    try {
        ...
        sqlapi.selectData('SELECT name AS sliderName,title AS sliderTitle,subtitle AS sliderSubtitle,img AS sliderImg FROM  mst_abouts ' + whereCondition, dataJson)
            .then(function (results) {
                finalData = results;
